...
This README tracks the current v3 pipeline. The runtime skill spec lives in [skills/spark-journal/SKILL.md](skills/spark-journal/SKILL.md), which is the source of truth for the latest command and setup behavior.

**Claude Code (recommended — auto-updates via marketplace):**
```
/plugin marketplace add arjun-chaudhry/spark-journal
/plugin install spark-journal
```

**Codex, Cursor, Copilot, Gemini CLI, or any of 50+ [Agent Skills](https://agentskills.io) hosts:**
```
npx skills add arjun-chaudhry/spark-journal -g
```
...
Reddit upvotes. X likes. YouTube transcripts. TikTok engagement. Polymarket odds backed by real money and insider information. That's millions of people voting with their attention and their wallets every day. /spark-journal searches all of it in parallel, scores it by what real people actually engage with, and an AI agent judge synthesizes it into one brief.

Google aggregates editors. /spark-journal searches people.
...
```
/spark-journal Peter Steinberger
```

You have a meeting tomorrow. You Google them. You get their LinkedIn from 2023. /spark-journal gives you what they're actually doing this month: joined OpenAI to work on Codex, fighting Anthropic's ban on third-party agents, shipping 23 PRs at 85% merge rate, building "LobsterOS" for cross-device agent control, and r/ClaudeCode hit 569 upvotes debating whether he's a hero or "insufferable." Scattered across X posts, Reddit threads, YouTube transcripts, and GitHub commits. None of it was on Google.

## Why this exists

Arjun built it to keep up in AI. Everything changes every day and the Reddit and X nerds are always on top of it first. He needed better prompts, and the training data was always months behind what the community had already figured out.

But it turned into something bigger. Now Arjun runs it before a sales call to know the last 30 days truth about a business. Before a meeting to read someone's recent tweets and podcast transcripts. Before a Disney World trip to know which rides are closed and what the community says about Genie+. Before building anything to know what problems people are actually hitting.

If you're meeting with a CEO, have you read all their tweets and YouTube transcripts from the last 30 days? Arjun has.
...
**Before a meeting.** `/spark-journal Peter Steinberger` - joined OpenAI's Codex team, fighting Anthropic's ban on third-party agents, 23 PRs merged at 85% merge rate on GitHub, building LobsterOS for cross-device agent control. r/ClaudeCode: "Ever since OpenClaw released, it was widely known that if you run it through anything other than the API, you were gonna get banned eventually" (227 upvotes). That's not on LinkedIn.

**To read hiring signals.** `/spark-journal Listen Labs --hiring-signals` - current jobs and careers pages become cited evidence for focus shifts: hiring into enterprise security, customer success, infrastructure, or product expansion. The report says what the hiring appears to signal, not what the roadmap will ship.

**To find the topic before it peaks.** Ask `/spark-journal what's exploding in AI agents?` and the skill switches to discovery mode: the engine sweeps Reddit category listings, Hacker News front/best stories, Digg's AI 1000 feed, and X when authenticated; your agent judges the nominations (names, junk filtering, content-worthiness) and writes podcast / X-article angles; then you get 5-10 velocity-ranked topics. Every result includes cross-source numbers, a momentum label, and a ready-to-run `/spark-journal "<topic>"` follow-up.

**When something drops.** `/spark-journal Kanye West` - UK blocked his visa, Wireless Festival canceled, sponsors fled. But BULLY debuted #2 on Billboard. Fantano came back from his "Yay sabbatical" to review it (653K views). SoFi Homecoming brought out Lauryn Hill and Travis Scott for 44 songs. Polymarket: "Will Kanye tweet again?" 86% Yes. 23 Reddit threads, 17 YouTube videos, 86K upvotes.

**To compare tools.** `/spark-journal OpenClaw vs Hermes vs Paperclip` - "These aren't competitors, they're layers." OpenClaw is the executor (351K GitHub stars, live), Hermes is the self-improving brain (31K stars), Paperclip is the org chart (49K stars). Star counts pulled live from the GitHub API, not stale blog posts. Side-by-side table with architecture, memory, security, best-for. Per @IMJustinBrooke: "OpenClaw = Charmander, Hermes = Charizard."

**To understand the world.** `/spark-journal Iran vs USA` - Day 38 of the war. Trump's Tuesday deadline for Iran to reopen the Strait of Hormuz. Two US warplanes downed. Oil at $126/barrel. The IEA called it "the largest supply disruption in the history of the global oil market." Polymarket: ceasefire by Dec 31 at 74%. 27 X posts, 10 YouTube videos, 20 prediction markets.

**Before a trip.** `/spark-journal Universal Epic Universe` - Expansion already under construction. "Project 680" permit filed. Fireworks show confirmed by infrastructure but unannounced. Wait times: Mine-Cart Madness averaging 148 minutes. No annual pass yet, and locals are frustrated. Stardust Racers down for refurbishment through April 5.

**To learn something fast.** `/spark-journal Nano Banana Pro prompting` - JSON-structured prompts are replacing tag soup. @pictsbyai's nested format prevents "concept bleeding." Edit-first workflow beats regeneration. Then it writes you a production prompt using exactly what the community said works.
...
|---------|---------|---------|
| **Claude Code** (recommended) | `/plugin marketplace add arjun-chaudhry/spark-journal` | Auto via marketplace, or `claude plugin update spark-journal@spark-journal` |
| **Grok** (xAI Build CLI) | `grok plugin marketplace add arjun-chaudhry/spark-journal` then `grok plugin install spark-journal` | `grok plugin update spark-journal` |
| **Codex, Cursor, Copilot, Gemini CLI, or any of 50+ [Agent Skills](https://agentskills.io) hosts** | `npx skills add arjun-chaudhry/spark-journal -g` | `npx skills update spark-journal -g` |
| **claude.ai** (web) | [Download `spark-journal.skill`](https://github.com/arjun-chaudhry/spark-journal/releases/latest/download/spark-journal.skill) and upload via claude.ai > Customize > Skills > + > Create skill > Upload a skill | Re-download and re-upload |
| **Claude Desktop** | [Download the `.mcpb` for your platform](https://github.com/arjun-chaudhry/spark-journal/releases/latest) and drag into Settings > Extensions | Re-download and drag the new bundle in |
| **OpenClaw** | `clawhub install spark-journal-official` | `clawhub update spark-journal-official` |
...
Recommended because the Claude Code marketplace handles updates for you — the plugin cache is versioned and auto-refreshes when a new release publishes. Run `claude plugin update spark-journal@spark-journal-skill` to force a check.

If you'd rather use the agent-skills install path on Claude Code, that's also supported:

```
npx skills add arjun-chaudhry/spark-journal -g -a claude-code
```

The native plugin and the `npx skills` install can coexist. Note that Claude Code does not dedupe across install methods: if you have both the marketplace plugin and the `npx skills` copy active, `/spark-journal` will show two entries. Use one install method per machine.

### Grok (xAI Build CLI)

[Grok Build](https://docs.x.ai/build/features/skills-plugins-marketplaces) (`grok`) installs spark-journal as a native plugin. Direct install tracks the repository:

```bash
grok plugin install arjun-chaudhry/spark-journal
```

Or add this repo as a marketplace source, then install by plugin name:

```bash
grok plugin marketplace add arjun-chaudhry/spark-journal
grok plugin install spark-journal
```

Add `--trust` to skip the install confirmation. Update with `grok plugin update spark-journal`. Grok also reads the Claude Code manifests for compatibility; the native `.grok-plugin/` pair is the first-class lane (and what an official [xAI marketplace](https://github.com/xai-org/plugin-marketplace) listing points at). `npx skills add` remains a valid cross-host fallback.
...
Codex desktop and other folder-mode hosts can work in ordinary folders as well as Git repos. Before first research, ask the host agent to run the bundled `scripts/spark-journal.py --preflight` from the loaded skill directory; in a source checkout, the equivalent command is `python3 skills/spark-journal/scripts/spark-journal.py --preflight`. It shows the config source, browser-cookie plan, planned writes, optional commands, and ignored project config without reading cookies, writing files, or running research.

By default this installs for whichever harness `npx skills` detects. To target a specific one (or multiple):

```bash
npx skills add arjun-chaudhry/spark-journal -g -a codex
npx skills add arjun-chaudhry/spark-journal -g -a cursor
npx skills add arjun-chaudhry/spark-journal -g -a gemini-cli
npx skills add arjun-chaudhry/spark-journal -g -a codex -a cursor
```

Update later with:

```bash
npx skills update spark-journal -g
```
...
List and remove with `npx skills list -g` and `npx skills remove spark-journal -g`.

### claude.ai (web)

1. [Download `spark-journal.skill`](https://github.com/arjun-chaudhry/spark-journal/releases/latest/download/spark-journal.skill) from the latest release
...
Claude Desktop installs `/spark-journal` as an MCP server via a `.mcpb` bundle (a one-click Model Context Protocol package).

1. Go to the [latest release](https://github.com/arjun-chaudhry/spark-journal/releases/latest) and download the `.mcpb` for your platform:
   - macOS Apple Silicon: `spark-journal-pp-mcp-darwin-arm64.mcpb`
   - macOS Intel: `spark-journal-pp-mcp-darwin-amd64.mcpb`
   - Linux x86_64: `spark-journal-pp-mcp-linux-amd64.mcpb`
...
**Keys don't sync with the Code skill.** Claude Desktop and Claude Code maintain separate credential stores by design. If you already configured `~/.config/spark-journal/.env` for the Code skill, you'll re-enter the same keys here once.
...
```bash
clawhub install spark-journal-official
```

For X/Twitter action workflows outside `/spark-journal` research, such as posting
...
optional companion path, not a spark-journal dependency or endorsement.

### Manual (developer)

```bash
git clone https://github.com/arjun-chaudhry/spark-journal.git
ln -s "$(pwd)/spark-journal/skills/spark-journal" ~/.claude/skills/spark-journal
```

The symlink keeps the install in sync with your working tree as you edit — no re-copy needed. For `claude.ai`, build the `.skill` file from source: `bash skills/spark-journal/scripts/build-skill.sh` produces `dist/spark-journal.skill`.

Reddit (with comments), Hacker News, Polymarket, and GitHub work immediately. Zero configuration. Run `/spark-journal` once and the setup wizard unlocks more sources in 30 seconds, including the free arXiv and Techmeme CLIs.
...
| Xiaohongshu (RED) | Run a logged-in x-mcp browser plugin or `xiaohongshu-mcp` service and opt in with `--search xhs` per run or `INCLUDE_SOURCES=xiaohongshu` in `.env`; spark-journal auto-probes `http://localhost:18060` then `http://host.docker.internal:18060`, or use `XIAOHONGSHU_API_BASE` for a custom URL | No spark-journal API key; depends on your local browser-session service |
...
skills/spark-journal/scripts/setup-keychain.sh

# Or store a single key by hand
security add-generic-password -a "$USER" -s spark-journal-XAI_API_KEY -w "xai-..."

# Inspect / clean up
skills/spark-journal/scripts/setup-keychain.sh --list
skills/spark-journal/scripts/setup-keychain.sh --delete XAI_API_KEY
```

Items are stored under service name `spark-journal-<KEY>` for the current user. On non-Darwin platforms the loader is a no-op, so there is no behaviour change for Linux/Windows users.

Already have keys under different Keychain service names? Set the non-secret `spark-journal_KEYCHAIN_ALIASES` mapping described in [CONFIGURATION.md](CONFIGURATION.md#reusing-existing-macos-keychain-items) instead of copying secrets.
...
**Where research files are saved.** `spark-journal_MEMORY_DIR` defaults to `~/Documents/spark-journal/` (Windows: `C:\Users\<you>\Documents\spark-journal\`). Override by setting that env var to any path in your shell, or `--save-dir <path>` per run. Use `--output <file>` when you need the rendered result at an exact path, using the format selected by `--emit`. Use `--save-suffix=<name>` to keep multiple variations of the same topic separate (e.g. per client). Each `--save-dir` run produces `<slug>-raw[-suffix].md`. Run `python3 skills/spark-journal/scripts/spark-journal.py --preflight` to review planned writes before a research run.

**Structured output for agents and workflows.** Ask `/spark-journal` for machine-readable JSON to receive the stable, versioned agent profile. For direct engine use in scripts or development, run `python3 skills/spark-journal/scripts/spark-journal.py "AI coding agents" --emit=json`; add `--json-profile=raw` only when you need the unversioned internal `Report` dump. See the [JSON export field reference and versioning policy](docs/reference/json-export.md).

**Topic-less discovery.** Ask `/spark-journal what's trending in AI agents?` to get a ranked discovery brief instead of researching a topic you already know - on an agent host this runs the three-command host-judged protocol (the model names topics, filters junk, scores worthiness, and writes the content angles). For direct engine use in scripts or cron, run `python3 skills/spark-journal/scripts/spark-journal.py --discover "AI agents"` (one-shot: deterministic topic names, no angles); add `--emit=json` for the versioned discovery contract. Discovery is mutually exclusive with a positional topic and `--drill`.

**Trend monitoring across runs.** The default mode produces a fresh markdown snapshot per run. To accumulate findings over time, add `--store` to persist into a SQLite database, then use [`scripts/watchlist.py`](skills/spark-journal/scripts/watchlist.py) for scheduled runs (with optional Slack / webhook delivery on new findings) and [`scripts/briefing.py`](skills/spark-journal/scripts/briefing.py) for daily / weekly digests. The full cadence pattern is in [CONFIGURATION.md](CONFIGURATION.md#trend-monitoring-store--watchlist--briefings).

**A subscribable research library.** Ask `/spark-journal` to build your library feed, or use `python3 skills/spark-journal/scripts/spark-journal.py library feed` directly for scripting and development. It turns saved briefs into `index.html`, a local Atom `feed.xml`, and readable brief pages. Add `--publish` only when you want the HTML index and brief pages hosted; publishing is explicit opt-in and public by default. To make the Atom feed subscribable, host the generated output directory on a static host such as GitHub Pages.

**Search everything you've researched.** Ask `/spark-journal search my library for MCP servers` or `/spark-journal have I researched MCP servers before?`. For direct engine use, run `python3 skills/spark-journal/scripts/spark-journal.py library search "MCP servers"`. Search is offline and deterministic: it incrementally indexes the same saved briefs used by the library feed, merges matching per-run store sightings, and groups results by topic and date. Fresh runs also surface a compact **From your library** section when prior research overlaps the current topic; set `spark-journal_LIBRARY_CONTEXT=off` to disable that passive context.
...
Published a recurring AI update, market watch, or wonderfully narrow obsession with spark-journal? Share the public library URL—or the Atom URL after hosting `feed.xml` on a static host—in [the community showcase thread](https://github.com/arjun-chaudhry/spark-journal/issues/532). Community feeds will be linked here as their owners submit them; the thread is the collection point in the meantime.
...
> "5 of the 10 trending repos on GitHub today are Claude tools. #1: arjun-chaudhry/spark-journal" -@yieldhunter95

## Open source

MIT license. No tracking. No analytics. Your research stays on your machine. 2,700+ tests.

Built with Python 3.12+, yt-dlp, Node.js (vendored Bird client for X search), and ScrapeCreators API. v3 engine architecture by [@j-sperling](https://github.com/j-sperling). Maintained by [@arjun-chaudhry](https://github.com/arjun-chaudhry).
...
<a href="https://star-history.com/#arjun-chaudhry/spark-journal&Date">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=arjun-chaudhry/spark-journal&type=Date&theme=dark" />
    <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/svg?repos=arjun-chaudhry/spark-journal&type=Date" />
    <img alt="Star History Chart" src="https://api.star-history.com/svg?repos=arjun-chaudhry/spark-journal&type=Date" />
  </picture>
</a>

---

**@spark-journal** · [github.com/arjun-chaudhry/spark-journal](https://github.com/arjun-chaudhry/spark-journal)
