...
Everything you can tune in `/spark-journal` without editing the engine source.
...
This is a focused **configuration reference** maintained alongside the engine. The runtime contract (the voice rules, the planner protocol, the LAWs the synthesizing model follows) lives in [`skills/spark-journal/SKILL.md`](skills/spark-journal/SKILL.md) - that file is authoritative when the two ever differ. This file's job is narrower: surface every knob a user or operator can turn, in one place, kept current with the code so client-facing setups stay reliable. New configuration knobs added to the engine should be reflected here in the same PR.
...
|---|---|---|
| Linux / macOS | `spark-journal_MEMORY_DIR` defaults to `~/Documents/spark-journal/` | set `spark-journal_MEMORY_DIR=/path` |
| Windows | `spark-journal_MEMORY_DIR` defaults to `C:\Users\<you>\Documents\spark-journal\` | set `spark-journal_MEMORY_DIR=C:\path` |
...
`.env` files don't travel between machines or harnesses, so set `spark-journal_MEMORY_DIR` explicitly in `~/.config/spark-journal/.env` once per host. The `/spark-journal` slash command works without it (the SKILL.md wrapper has its own default), but **bare engine invocations** — `python3 scripts/spark-journal.py ...` from cron jobs, scripts, or agents that bypass the wrapper — silently no-op the file save unless the engine sees the env var. Mirrors the `spark-journal_STORE` env-or-flag convention.

```bash
# ~/.config/spark-journal/.env  (pick ONE — uncomment the line that matches your OS)
spark-journal_MEMORY_DIR=~/Documents/spark-journal                      # POSIX — defaults to this path when unset
# spark-journal_MEMORY_DIR=C:\Users\<user>\Documents\spark-journal      # Windows
# spark-journal_LIBRARY_OWNER=Your Name                              # Optional Atom feed author
# spark-journal_LIBRARY_CONTEXT=off                                  # Disable prior-run context (default: on)
```
...
- `--save-dir <path>` - one-off output location. **Flag wins over env var.** If neither flag nor env var is set, the engine does not write a file (DB persistence is independent — see `spark-journal_STORE` below).
...
- `library feed` - scan `spark-journal_MEMORY_DIR` plus `~/.local/share/spark-journal/briefs/`, then write a self-contained `index.html`, valid Atom `feed.xml`, and browser-ready pages under `briefs/`. The index is reverse-chronological and grouped by topic. For direct engine use: `python3 skills/spark-journal/scripts/spark-journal.py library feed`; use `--save-dir <path>` to scan and write another library directory.
...
- `library search "<query>"` - incrementally sync `spark-journal_MEMORY_DIR` and `~/.local/share/spark-journal/briefs/` through the shared library scanner, then run offline SQLite FTS5 across those briefs plus dated per-run sightings in `~/.local/share/spark-journal/research.db`. Results are grouped by topic run. The sibling search index lives at `~/.local/share/spark-journal/library.db`; hand edits, renames, and deletes are picked up on sync, and a corrupt index is rebuilt automatically.
- `spark-journal_LIBRARY_OWNER=<name>` - optional feed-level Atom author. Defaults to `spark-journal research library`.
- `spark-journal_LIBRARY_CONTEXT=on|off` - controls passive prior-run context on fresh research reports. It defaults to `on`; matching saved research appears in a short `From your library` section. Set `off` to skip the local index read and leave reports unchanged. Mock runs, eval replays, and internal fan-out subruns do not load library context, keeping fixtures deterministic.
- `--publish-password <password>` - optional shared password for `--publish-html` or `library feed --publish`. Prefer `spark-journal_PUBLISH_PASSWORD=<password>` instead so the password is not visible in the process list or shell history. Use a unique non-personal password; never reuse the user's own password. The provider's update key is treated as secret and is not written to stdout, HTML, raw output, or `.publish.json` metadata.
...
The footer line `📎 Raw results saved to ${spark-journal_MEMORY_DIR:-$HOME/Documents/spark-journal}/<slug>-raw.md` is the canonical pointer; if it shows backslashes on Windows update past v3.1.1.

Every completed research pass writes a structured `last-report.json` cache beside `last-run.json`. HTML follow-up renders use it so `--emit=html --synthesis-file` can reuse report metadata/footer without fetching sources again; `--drill <target>` uses it as the grounded starting point for targeted re-research; bare `--verify-freshness` updates only the cached report's claim verdicts. Reuse is intentionally short-lived: `spark-journal_REPORT_CACHE_TTL_SECONDS` defaults to `3600` (one hour). Set it to another integer number of seconds to tune the window, or `0` to disable report-cache reuse and post-run follow-ups.

---

## First-run onboarding

On the very first `/spark-journal` run (no `~/.config/spark-journal/.env`, or `SETUP_COMPLETE` not set), the skill runs a consent-driven onboarding the model drives in chat. It takes one of two forms depending on the host:
...
Re-run onboarding by deleting `~/.config/spark-journal/.env`. The mechanical work lives in `scripts/lib/setup_wizard.py`; the consent conversation and both host flows are specified in `skills/spark-journal/SKILL.md` Step 0. The original v3.0.0 wizard is captured at `docs/reference/old-nux-wizard-v3.0.0.md`.
...
1. **`~/.config/spark-journal/.env`** at the user level (global default) - loaded by default.
2. **`.claude/spark-journal.env`** in the current project directory (project-scoped) - loaded only when trusted by setting `spark-journal_TRUST_PROJECT_CONFIG=1` in the process environment or global config.

Override the global location with `spark-journal_CONFIG_DIR=/path` (or `spark-journal_CONFIG_DIR=""` for no-config mode). File permissions should be `600` on POSIX hosts - the engine warns on every run if they aren't.

The project-scoped file is useful for **intentional per-client setups**: drop a `.claude/spark-journal.env` into each client folder (`SCRAPECREATORS_API_KEY`, `INCLUDE_SOURCES`, `spark-journal_MEMORY_DIR`, `BSKY_HANDLE`, etc), then opt in with `spark-journal_TRUST_PROJECT_CONFIG=1` from your shell or `~/.config/spark-journal/.env`. Folder-mode hosts such as Codex desktop do not trust hidden project config by default, and discovery stops at the git root so unrelated parent folders cannot silently influence runs.

**`spark-journal_API_KEY`** + **`spark-journal_API_BASE`** - optional remote-API backend. Set BOTH to route research through a remote API endpoint instead of running the local sources: `spark-journal_API_BASE` is the endpoint (there is no built-in default), and `spark-journal_API_KEY` is the bearer key for it. When both are set (and `--mock` is not passed), the engine submits the topic to that endpoint, polls with progress on stderr, and prints the server's report; none of the per-source keys below are used for that run. A configured local corpus is the privacy exception: the engine bypasses the hosted backend and runs locally rather than forwarding file-derived input. Non-default `--register` selections are forwarded with the request so server-side synthesis uses the same audience preset. Leave either unset to run local sources exactly as normal. Unlike the other keys here, these two are read only from the **process environment** (export them in your shell or host config) - they are deliberately not loaded from the `.env` files above, so a project-scoped `.env` can never silently redirect research to a remote endpoint. The remote endpoint does not return the local `Report` needed for the versioned agent JSON profile; use `--emit=json --json-profile=raw` for its existing server-response JSON contract.

### Local corpus (your files)

Register persistent directories with `spark-journal_CORPUS_DIRS`. Separate paths with `:` on macOS/Linux (the platform path separator is `;` on Windows):

```bash
# ~/.config/spark-journal/.env
spark-journal_CORPUS_DIRS=~/notes:~/meeting-transcripts
# spark-journal_CORPUS_IN_EXPORT=1  # explicit agent-JSON opt-in; off by default
```

The slash-command experience remains primary: ask `/spark-journal` to include your registered notes. For direct engine scripting or development, the equivalent one-off invocation is:

```bash
python3 skills/spark-journal/scripts/spark-journal.py "MCP servers" \
  --corpus ~/notes --corpus ~/meeting-transcripts
```

**Privacy:** corpus files are read locally, never sent through a source HTTP client, never forwarded to `spark-journal_API_BASE`, never included in remote reranker/fun-scoring prompts, and do not consume network-source concurrency or retry budget. Matches appear in a badged **From your files** section. Corpus candidates are removed from `--publish-html`, `library feed --publish`, and the versioned agent JSON export by default, including corpus-derived cluster titles and source outcomes. Set `spark-journal_CORPUS_IN_EXPORT=1` only when you intentionally want corpus results in the agent JSON written to local stdout/files. The unversioned `--json-profile=raw` debug dump remains a full local report and can contain corpus text; do not redirect it to an external system unless that is intentional. Extracted text is cached by file mtime in `~/.config/spark-journal/corpus-cache.json` with mode `0600`; a corpus-bearing `last-report.json` cache is also tightened to `0600`. Delete either cache at any time to clear it.

**Source-by-source** - what each key unlocks:

| Source | Key(s) | Required for | Free tier |
|---|---|---|---|
| Local corpus | `--corpus <dir>` or `spark-journal_CORPUS_DIRS` | private `.md`/`.txt`; `.pdf` when `pdftotext` is on PATH | yes (offline) |
| Reddit (public) | none (default free keyless path). With `SCRAPECREATORS_API_KEY`: empty-only search backup by default; `spark-journal_REDDIT_SC_MIN_ITEMS=<N>` backfills thin free runs; `spark-journal_REDDIT_BACKEND=scrapecreators` pins SC primary with free fallback | always on; SC knobs require `SCRAPECREATORS_API_KEY` | yes |
...
| Digg | `digg-pp-cli` on PATH (auto-installed during first-run setup via `npx -y @arjun-chaudhry/printing-press-library@0.1.16 install digg --cli-only`; binary defaults to `$HOME/.local/bin` — Hermes/OpenClaw agent subprocesses must inherit that dir on PATH for Digg to activate; prior pp-digg installs use the same path) | always on if `digg-pp-cli` on PATH | yes (free, keyless, read-only) |
| arXiv | `arxiv-pp-cli` on PATH (auto-installed during first-run setup via `npx -y @arjun-chaudhry/printing-press-library@0.1.16 install arxiv --cli-only`) | always on if `arxiv-pp-cli` on PATH; fires on research/technical topics and stays quiet otherwise (relevance + 365-day recency gating) | yes (free, keyless) |
...
| Instagram | `SCRAPECREATORS_API_KEY` + `INCLUDE_SOURCES` contains `instagram` | Instagram Reels | 10K free calls; raise `spark-journal_TRANSCRIPT_TIMEOUT` (default 30s) if SC is slow on your network |
...
| Xiaohongshu (RED) | logged-in x-mcp browser plugin or `xiaohongshu-mcp` service; optional `XIAOHONGSHU_API_BASE` for custom URLs | requested-only via `--search xhs` or `--search xiaohongshu`; auto-probes `http://localhost:18060` then `http://host.docker.internal:18060` | no spark-journal API key; depends on your local browser-session service |
...
**YouTube transcript tuning.** `spark-journal_YT_SUB_LANGS` controls the comma-separated caption-language priority passed to yt-dlp and defaults to `en,es,pt`. When `SCRAPECREATORS_API_KEY` is available, yt-dlp uses one fast attempt before the paid fallback; set `spark-journal_YT_TRANSCRIPT_FAST_TIMEOUT` to the number of seconds allowed for that attempt when a throttled host needs longer than the 12-second default. A VTT completed before the timeout is reused rather than discarded.
...
# spark-journal_PERPLEXITY_MODE=sonar  # sonar | search | both
# spark-journal_PERPLEXITY_MODEL=sonar-pro  # sonar | sonar-pro | sonar-reasoning-pro
...
After editing: `chmod 600 ~/.config/spark-journal/.env` (or `chmod 600 .claude/spark-journal.env` if using the project-scoped variant).

**Troubleshooting:** if a source you expected to see isn't appearing in results, run `python3 scripts/spark-journal.py --preflight` for a human permission summary or `python3 scripts/spark-journal.py --diagnose` for full JSON diagnostics. Both are safe: they report source availability, config source, browser-cookie plan, external command availability, write destinations, and ignored untrusted project config without reading browser cookies or running live provider probes.
...
`spark-journal_PERPLEXITY_MODE` controls normal `perplexity` source runs:
...
`--deep-research` ignores `spark-journal_PERPLEXITY_MODE` and uses `sonar-deep-research`. With `PERPLEXITY_API_KEY`, it submits to Perplexity's async Sonar endpoint and polls with a hard wall-clock timeout. The async request uses a deterministic idempotency key derived from the request body. If the request is still running at timeout, fails remotely, or polling hits a transport/rate-limit error after the async id exists, the raw artifact records the async request id, idempotency key, last status, lifecycle timestamps returned by Perplexity, poll count, and timeout/error fields so you can inspect or resume by id outside the run. With only `OPENROUTER_API_KEY`, it keeps the OpenRouter synchronous fallback.

Perplexity-specific env vars:

| Env var | Default | Applies to | Notes |
|---|---|---|---|
| `spark-journal_PERPLEXITY_MODE` | `sonar` | normal Perplexity source runs | `sonar`, `search`, or `both`; `search` and `both` require `PERPLEXITY_API_KEY`. |
| `spark-journal_PERPLEXITY_MODEL` | `sonar-pro` | direct Sonar only | Supported: `sonar`, `sonar-pro`, `sonar-reasoning-pro`. `--deep-research` forces `sonar-deep-research`. |
| `spark-journal_PERPLEXITY_MAX_RESULTS` | `10` | Search API | Clamped to Perplexity's 1..20 range. |
| `spark-journal_PERPLEXITY_SEARCH_CONTEXT_SIZE` | provider default | Search API | `low`, `medium`, or `high`; omitted unless set. |
| `spark-journal_PERPLEXITY_SEARCH_MODE` | provider default | direct Sonar | `web`, `academic`, or `sec`. |
| `spark-journal_PERPLEXITY_DOMAIN_FILTER` | unset | Search API and direct Sonar | Comma-separated domains, max 20. |
| `spark-journal_PERPLEXITY_LANGUAGE_FILTER` | unset | Search API and direct Sonar | Comma-separated ISO 639-1 language codes, max 20. |
| `spark-journal_PERPLEXITY_COUNTRY` | unset | Search API | Two-letter country code such as `US`. |
| `spark-journal_PERPLEXITY_RECENCY_FILTER` | unset | Search API and direct Sonar | `hour`, `day`, `week`, `month`, or `year`. |
| `spark-journal_PERPLEXITY_REASONING_EFFORT` | unset | direct Sonar | `minimal`, `low`, `medium`, or `high`. |
| `spark-journal_PERPLEXITY_DEEP_TIMEOUT_SECONDS` | `600` | direct async Deep Research | Wall-clock polling deadline. |
...
(`.claude/spark-journal.env`) > global config (`~/.config/spark-journal/.env`) >
macOS Keychain > `pass`(1). The SessionStart status hook also checks for
Keychain item **presence** under `spark-journal-<KEY>` without reading secret
...
|---|---|---|---|
| macOS | Keychain | `scripts/setup-keychain.sh` | service name `spark-journal-<KEY>` |
| Linux / Unix (anywhere `pass` exists, incl. macOS) | [`pass`(1)](https://www.passwordstore.org/) | `scripts/setup-pass.sh` | pass path `spark-journal/<KEY>` |
...
under a different prefix, point the loader at it with `spark-journal_PASS_PREFIX`
...
```bash
export spark-journal_PASS_PREFIX="secrets/spark-journal/"   # default: spark-journal/
```
...
metadata in `spark-journal_KEYCHAIN_ALIASES`. The loader still checks
`spark-journal-<KEY>` first; aliases are fallback lookups only.

```bash
# ~/.config/spark-journal/.env
spark-journal_KEYCHAIN_ALIASES={"XAI_API_KEY":{"account":"keychain-user","service":"existing-xai-api-key"},"BRAVE_API_KEY":"existing-brave-api-key"}
```
...
```bash
spark-journal_KEYCHAIN_ALIASES={"XAI_API_KEY":[{"account":"keychain-user","service":"existing-xai-api-key"},{"service":"last-resort-xai"}]}
```
...
Write `spark-journal_KEYCHAIN_ALIASES` as a single-line JSON value in `.env`.
...
### Default source set (`spark-journal_DEFAULT_SEARCH`)
...
```bash
spark-journal_DEFAULT_SEARCH=reddit,x,youtube,hn
```

Accepts the same comma-separated names and aliases as `--search` (`web` → grounding, `hn` → hackernews, `bsky` → bluesky, `xhs` → xiaohongshu). Precedence: an explicit `--search` on the command line always wins; `spark-journal_DEFAULT_SEARCH` applies only when the flag is omitted; when neither is set, per-query behavior is unchanged. `INCLUDE_SOURCES` / `EXCLUDE_SOURCES` keep their existing additive/subtractive roles on whichever set is selected.

### Audience register (`spark-journal_REGISTER`)
...
```bash
spark-journal_REGISTER=exec  # default | exec | dev | creator | eli5
```

An explicit `--register` wins over `spark-journal_REGISTER`; the environment/config value defaults to `default`. Presets are intentionally named and bounded - arbitrary prompt or template files are not accepted. Existing `ELI5_MODE=true` configurations continue to resolve to the `eli5` register when no explicit register is selected, but new configuration should use `spark-journal_REGISTER=eli5`.

---

## Reasoning provider priority

`/spark-journal` needs one reasoning model for planning + reranking when you don't pass `--plan` yourself. Auto-detect priority (set `spark-journal_REASONING_PROVIDER=<name>` to pin one):
...
When you invoke `/spark-journal` from Claude Code, Codex, or Gemini, the host model **is** the reasoning provider for plan + synthesis - you don't need any of the keys above unless you also run the script headlessly (cron, CI, watchlist).
...
| Var | Effect |
| --- | --- |
| `spark-journal_NATIVE_SEARCH=1` | Tells the engine your agent session has host-side web search; suppresses the keyless floor. Set automatically by the skill when web search is available. Leave unset when the agent has no web-search tool so the floor runs. |
| `spark-journal_SEARXNG_URL=<base-url>` | Optional. A SearXNG instance used as the keyless-search fallback rung when DuckDuckGo returns nothing. |
| `spark-journal_TRUSTPILOT_NO_BROWSER=1` | Optional. Truthy value disables the Trustpilot source's one-time headless-Chrome WAF-cookie harvest, so an automated/headless run (cron, CI, the eval harness) never spawns a browser. Trustpilot still degrades to empty gracefully. |
...
```bash
python3 skills/spark-journal/scripts/spark-journal.py "Listen Labs" --hiring-signals
```
...
```bash
python3 skills/spark-journal/scripts/spark-journal.py doctor              # four-state audit (text)
python3 skills/spark-journal/scripts/spark-journal.py doctor --json       # machine contract
python3 skills/spark-journal/scripts/spark-journal.py doctor --cached     # serve the cached report while fresh
python3 skills/spark-journal/scripts/spark-journal.py doctor --postmortem # what actually broke on the last run
python3 skills/spark-journal/scripts/spark-journal.py doctor --probe      # bounded live test (free/CLI sources)
```

Slash-command form: `/spark-journal doctor`. Reporting problems is a successful run — the exit code is always 0, no browser cookies are read, and no secret values appear anywhere (key presence is booleans only). Backends within a chained source are probed sequentially with a 5-second budget per binary probe, so a chained source's worst-case check time is additive across its backends (only reached when several binaries hang at once).
...
Every live run writes its JSON result to `~/.config/spark-journal/doctor-cache.json` (beside `last-run.json`; honors `spark-journal_CONFIG_DIR`). `doctor --cached` returns that stored report when it is younger than the TTL, and falls through to a live run — rewriting the cache — when it is stale, absent, or corrupt. The cache also self-invalidates on configuration change: the payload carries a schema stamp plus a fingerprint of non-secret config signals (which credentials are present as booleans, the `spark-journal_X_BACKEND` / `spark-journal_REDDIT_BACKEND` pin values, and `INCLUDE_SOURCES`), so adding or removing a key, changing a pin, or toggling an opt-in source makes the next `--cached` call run live — no raw secret ever enters the fingerprint or the file. Every report also carries `from_cache` (true/false) and `generated_at` (when the report was built), in the `--json` top level and as a final `generated: … (cached|live)` text line, so you can always tell how old a cached answer is. A failed cache write is never fatal — doctor prints a one-line stderr warning and continues. An explicit `doctor` without `--cached` always runs live and refreshes the cache.

| Var | Effect |
| --- | --- |
| `spark-journal_DOCTOR_TTL` | Freshness window for `doctor --cached`, in **seconds**. Defaults to `900` (15 minutes). `0` makes every `--cached` call run live. |
| `spark-journal_DOCTOR_PROBE_TIMEOUT` | Per-source deadline (**seconds**) for `doctor --probe` live checks. Defaults to `10`. Caps each concurrent probe so a slow source cannot hang the command. |
| `spark-journal_X_BACKEND` | Pins the X backend (`xai` / `bird` / `xurl` / `xquik`); doctor renders the pin and predicts "will use" accordingly. |
| `spark-journal_REDDIT_BACKEND` | `scrapecreators` makes ScrapeCreators the primary Reddit backend; doctor renders Reddit's conditional routing with the pin applied. |
| `spark-journal_REDDIT_SC_MIN_ITEMS` | Integer thinness floor for ScrapeCreators Reddit **search** backfill. Default `0` = empty-only (free path keeps any non-empty result; no credit spend). Set above `0` to backfill when free yield is below that count; merged results dedupe by post id. Requires `SCRAPECREATORS_API_KEY`. Ignored when `spark-journal_REDDIT_BACKEND=scrapecreators` (SC is already primary). |
...
| Var | Effect |
| --- | --- |
| `spark-journal_STRICT_EXIT` | Truthy (`1`/`true`/`yes`/`on`): the engine exits `3` when any source outcome is neither `ok`, `no-results`, nor `skipped-unconfigured`. A one-line `strict-exit: degraded sources: ...` note goes to stderr. Default (unset): exit `0`, unchanged behavior. |

Exit codes with the flag on: `0` clean run, `3` completed-but-degraded (report was produced), non-zero others unchanged (hard failures). Same hybrid pattern as `spark-journal_DEBUG` — works shell-exported or in `.env`.
...
**Always-on alternative:** set `spark-journal_DEBUG=true` in your `.env` or export it from your shell. The flag still works as before; the env var is purely additive — works whether shell-exported or set in `.env`.
...
Adding `--store` to any run persists every finding to a SQLite database (default at `~/.local/share/spark-journal/research.db`). Findings dedupe on the `source_url` column (UNIQUE constraint), so the same URL across runs updates the existing row instead of creating a duplicate. The markdown file still saves; the SQLite is the time-series substrate.

**Always-on alternative:** set `spark-journal_STORE=1` in your `.env` instead of remembering `--store` on every invocation. The flag still works as before; the env var is purely additive. Same hybrid pattern as `spark-journal_DEBUG` — works whether shell-exported or in `.env`.

Relevant tables: `topics`, `research_runs`, `findings`, `settings`. Schema: [`scripts/store.py`](skills/spark-journal/scripts/store.py).

### Discovery topic queue (`spark-journal_DISCOVERY_QUEUE`)
...
| Var | Effect |
| --- | --- |
| `spark-journal_DISCOVERY_QUEUE` | Set to `off` to disable queue writes and card annotations. Any other value (or unset) keeps the queue on. Works shell-exported or in `.env`. |
| `spark-journal_ENRICH_BUDGET_SECONDS` | Wall-clock budget (seconds) for the deep-tier per-topic research batch on the discovery resume leg (`--discover --judgments <file>`). Default `450`; unset/invalid/non-positive values fall back to it. The one-shot `--discover` path keeps its fixed quick-tier 240s budget regardless. Works shell-exported or in `.env`. |

Manage the queue from the engine CLI:

```bash
# Uncovered surfaced topics (name, domain, surface_count, last_surfaced, status)
python3 skills/spark-journal/scripts/spark-journal.py queue list
...
python3 skills/spark-journal/scripts/spark-journal.py queue cover "Gemma 4 chat templates"
```
...
[`scripts/watchlist.py`](skills/spark-journal/scripts/watchlist.py) manages topics that should be researched on a schedule. Subcommands: `add`, `remove`, `list`, `run-one`, `run-all`, `config`. Built-in delivery to Slack incoming webhooks (`hooks.slack.com/...`) or any HTTPS endpoint, fired only when new findings appear.
...
[`scripts/briefing.py`](skills/spark-journal/scripts/briefing.py) reads the SQLite store and emits structured data the agent then synthesizes into prose. Modes: `generate` (daily), `generate --weekly`, `show [--date DATE]` (display a saved briefing). Briefs save to `~/.local/share/spark-journal/briefs/`.

### Recommended cadence pattern

| Step | Cadence | Command |
|---|---|---|
| Baseline | one-time per topic | `/spark-journal "<topic>" --days=30 --store` |
...
`skills/spark-journal/SKILL.md` tree as plugin metadata without maintaining a separate Codex copy.
...
so xAI's Grok Build CLI (`grok`) can install spark-journal as a native plugin. Grok also reads the
...
arjun-chaudhry/spark-journal-skill` tracks HEAD — the same pattern as the Codex catalog. `npx skills add`
remains a valid cross-host fallback.

### 1. Trusted per-client `.claude/spark-journal.env`

When each client has its own working directory, drop a `.claude/spark-journal.env` into the client folder and opt in with `spark-journal_TRUST_PROJECT_CONFIG=1` from your shell or global `~/.config/spark-journal/.env`. The skill loads the project file only after that trust signal. Typical contents:

```bash
spark-journal_MEMORY_DIR=C:\Users\<you>\Clients\acme\Research\spark-journal
SCRAPECREATORS_API_KEY=<acme-scoped-key-or-shared>
INCLUDE_SOURCES=tiktok,instagram
BSKY_HANDLE=<acme-bluesky-handle>.bsky.social
```

`cd` into the client folder, run `/spark-journal <topic>` as normal, no wrappers. Combine with `--save-suffix=<client-slug>` per run if you also need to differentiate filenames within that folder.
...
function Run-L30D-Client {
    ...
    $env:spark-journal_MEMORY_DIR = "C:\Users\$env:USERNAME\Clients\$ClientSlug\Research\spark-journal"
    /spark-journal @Args --save-suffix=$ClientSlug
}
...
l30d-client() {
    ...
    spark-journal_MEMORY_DIR="$HOME/Clients/$client/Research/spark-journal" \
        /spark-journal "$@" --save-suffix="$client"
}
...
[`scripts/lib/categories.py`](skills/spark-journal/scripts/lib/categories.py) holds a table of `(category_id, trigger_keywords, peer_subreddits)`. If a client lives in a vertical that isn't covered (legal-tech, real-estate-tech, B2B HR SaaS), add a row. Pure data, no logic.
...
Experimental customizations live on a private companion repo (`arjun-chaudhry/spark-journal-skill-private`) installed as `/spark-journal-beta`. Never ship beta-only changes to the public marketplace without a review PR against the public repo. Workflow guide: `BETA.md` in the private repo.
...
- The CLI flag surface: `python3 scripts/spark-journal.py --help`
- The skill contract (voice, LAWs, pre-flight protocol): [`skills/spark-journal/SKILL.md`](skills/spark-journal/SKILL.md)
