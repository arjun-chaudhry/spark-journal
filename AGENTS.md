...
## Structure
- `skills/spark-journal/SKILL.md` — canonical skill definition / runtime spec the model reads when the slash command fires
- `skills/spark-journal/scripts/spark-journal.py` — main research engine
- `skills/spark-journal/scripts/lib/` — search, enrichment, rendering modules
- `skills/spark-journal/scripts/lib/vendor/bird-search/` — vendored X search client
...
## Orientation
- This is an Agent Skills package, not a CLI tool. The product is the slash-command-invoked skill (`/spark-journal <topic>` in most harnesses); `scripts/spark-journal.py` is implementation. Claude Code is the most common host but not the only one — features must work across every harness the skill installs into.
...
- README and PR examples show `/spark-journal <topic>` first. Direct CLI invocation (`python3 scripts/spark-journal.py ...`) is a fallback for scripting, cron, and dev-time engine testing; label it as such, never as the primary path.
- Slash commands don't pass shell mechanics through. `/spark-journal OpenClaw --emit=html | pbcopy` is invalid in any harness — either use the slash form (no flags or pipes; let the model translate user intent into engine flags) or use the direct CLI form (full `python3 ...` with explicit flags and a real shell).

## Commands
```bash
# Dev/fallback: direct engine invocation (scripting, cron, or engine testing only).
# Saves to $spark-journal_MEMORY_DIR when set in shell or ~/.config/spark-journal/.env;
# add --save-dir <path> for a one-off override. Mirrors spark-journal_STORE convention.
python3 skills/spark-journal/scripts/spark-journal.py "test query" --emit=compact
...
- One-time setup: `npx skills add . -g -y` copies the skill into `~/.agents/skills/<name>/` (real directory) and, for harnesses that support symlinked skill dirs, drops a per-host symlink pointing at that copy. **Working-tree edits do NOT propagate automatically** — the `~/.agents/skills/<name>/` copy is frozen at install time. To sync after edits, re-run `npx skills add . -g -y`. For live-edit on a dev machine, replace the install copy with a symlink to the working tree: `ln -sfn "$PWD/skills/spark-journal" ~/.agents/skills/spark-journal` (run from the repo root).
- Git remote: origin = public (`arjun-chaudhry/spark-journal-skill`)
...
- adding a new env var (e.g. `spark-journal_*`, `BSKY_*`, `*_API_KEY`)
...
The repo doubles as a native Grok Build plugin via `.grok-plugin/plugin.json` + `.grok-plugin/marketplace.json`. Grok also reads `.claude-plugin/*` for compatibility; the native pair is the first-class lane and what an official xAI marketplace listing points at. The self-hosted catalog uses a bare Git URL source (`{"source":"url","url":"https://github.com/arjun-chaudhry/spark-journal-skill.git"}`) so `grok plugin marketplace add arjun-chaudhry/spark-journal-skill` tracks HEAD — not a self-referential local `path: "."` (Grok does not enumerate those). Version lockstep with Claude/Codex/Gemini manifests is enforced by `tests/test_plugin_contract.py`. Validate with `grok plugin validate .`.

## Submitting to the xAI plugin marketplace

Getting spark-journal into xAI's official catalog (`xai-org/plugin-marketplace`) is an outbound PR to *their* repo — an index that only points at our source, so nothing of spark-journal is vendored there. Do this **after** the change you want to ship has merged to `main`: the entry pins a commit that must already exist.
...
2. Get the commit to pin — a full 40-char lowercase SHA; a branch, tag, or short SHA is rejected by their validator:
   ```bash
   git ls-remote https://github.com/arjun-chaudhry/spark-journal-skill.git HEAD
   ```
3. Add one entry to their `.grok-plugin/marketplace.json` under `plugins[]`, a remote source pinned to that SHA:
   ```json
   {
     ...
     "source": {
       "source": "url",
       "url": "https://github.com/arjun-chaudhry/spark-journal-skill.git",
       "sha": "<full-40-char-sha-from-step-2>"
     },
     "homepage": "https://github.com/arjun-chaudhry/spark-journal-skill",
     "keywords": ["spark-journal", "last 30 days"]
   }
   ```
...
Do not confuse this with our own `.grok-plugin/marketplace.json`: that file makes this repo directly addable as a Grok marketplace (`grok plugin marketplace add arjun-chaudhry/spark-journal-skill`) and uses a **bare URL** source (no SHA) so it tracks HEAD; the xAI entry above lives in *their* repo and uses a **remote** source pinned to a SHA.

## Beta channel

Experimental changes get tested on `arjun-chaudhry/spark-journal-skill-private`, which installs as a parallel `/spark-journal-beta` slash command. Beta-only changes never ship to public without a review PR here. Workflow guide lives at `BETA.md` in the private repo. Plan that established this setup: `docs/plans/2026-04-17-005-feat-beta-skill-from-private-repo-plan.md`.
