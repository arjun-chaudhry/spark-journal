"""HTML rendering for shareable spark-journal reports."""
...
CANONICAL_BOUNDARY_PATTERN = re.compile(r"\n?---\n# END OF spark-journal CANONICAL OUTPUT.*$", re.DOTALL)
...
<title>spark-journal · __TITLE__</title>
<style>
__CSS__
</style>
</head>
...
LIBRARY_BRIEF_MARKER = "<!-- generated by spark-journal library feed -->"
...
def render_library_index(
    ...
    *,
    ...
    if feed_url is None:
        parts.append('<p>Saved spark-journal briefs, newest first.</p>')
    else:
        parts.extend([
            '<p>Saved spark-journal briefs, newest first. Follow the Atom feed to keep up.</p>',
            f'<p><a class="subscribe" href="{html.escape(feed_url, quote=True)}">Subscribe via Atom</a></p>',
        ])
    parts.append('</header>')
    if not entries:
        parts.append('<section class="library-empty"><h2>No saved briefs yet</h2><p>Run spark-journal research and this library will fill itself.</p></section>')
    ...
    colophon = '<footer class="colophon">Generated locally by <strong>spark-journal</strong>.</footer>'
...
def _markdown_to_html(md: str) -> str:
    ...
    while index < len(lines):
        ...
        if stripped.startswith("🌐 spark-journal"):
            flush_paragraph()
            close_list()
...
def _protect_engine_footers(md: str) -> tuple[str, dict[str, str]]:
    ...
    def replace(match: re.Match[str]) -> str:
        token = f"__spark-journal_ENGINE_FOOTER_{len(footers)}__"
...
def _wrap_engine_footer(body: str) -> str:
    ...
    return re.sub(
        r"__spark-journal_ENGINE_FOOTER_\d+__",
        replace,
        body,
    )
...
def _build_colophon(report: schema.Report, *, topic: str | None = None) -> str:
    ...
    rerun = html.escape(f"/spark-journal {display_topic}")
    return (
        '<div class="colophon">\n'
        f"  Generated {generated} by /spark-journal v{html.escape(version)} · topic: {escaped_topic}<br>\n"
