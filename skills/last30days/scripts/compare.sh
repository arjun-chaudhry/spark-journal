...
# Usage: bash skills/spark-journal/scripts/compare.sh "Kanye West"
#
# Runs /spark-journal (public release) and /spark-journal-beta (private beta)
...
if [ $# -eq 0 ]; then
  echo "Usage: bash skills/spark-journal/scripts/compare.sh <topic>"
  echo "  Example: bash skills/spark-journal/scripts/compare.sh Kevin Rose"
  exit 1
fi
TOPIC="$*"
...
echo "[1/2] Running /spark-journal (public release)..."
echo "  This takes 2-4 minutes..."
claude -p "/spark-journal $TOPIC" > /dev/null 2>&1 || true
...
# Run 2: private beta
echo "[2/2] Running /spark-journal-beta (private beta)..."
echo "  This takes 2-4 minutes..."
claude -p "/spark-journal-beta $TOPIC" > /dev/null 2>&1 || true
...
echo "  🧪 spark-journal-beta · branch <name> · synced $DATE"
